...
for iters in range(6):
    ...
    '''
    # Models ### update
    Wx = np.loadtxt('./rnn_persistence_ei_varblanks_weights_5/Wx5676.csv')
    Wh = np.loadtxt('./rnn_persistence_ei_varblanks_weights_5/Wh5676.csv')
    Wo = np.loadtxt('./rnn_persistence_ei_varblanks_weights_5/Wo5676.csv')
    Bi = np.loadtxt('./rnn_persistence_ei_varblanks_weights_5/bi5676.csv')
    Bo = np.loadtxt('./rnn_persistence_ei_varblanks_weights_5/bo5676.csv')
    ...
    #np.save('y_full_data_exp_5_modified.npy', N>0.5)
    ...
    while epoch<num_epochs-1:
        ...
        if epoch>test_epoch_start or epoch%10==0:  

            for j in range(5):
                r = np.random.randint(1,100)
                X_test, y_test = X_test[:100], y_test[:100] ### update
                feed_dict_={rnn._inputs: X_test, y: y_test}
        ...
        if epoch%1 == 0 :
            ...
            np.savetxt('loss_rnn_ei_cd_%d_fc_RNN.csv'%(iters),data)
