...
for trial_id in range(0,40):
	for num in (np.array([0,1,2])).tolist():
		...
		while(found == False and over == False):
			...
			if (counter%2): SC_saliency =  scale( ((SC_diff<0.45)*vpi_sal_real[jj[0]] + (SC_diff>0.45)*0.95) , 0, 1)
			else: SC_saliency =  scale( ((SC_diff<0.45)*vpi_sal_real[jj[0]] + (SC_diff>0.45)*0.95) , 0, 1)
		...
		np.savetxt('saccade_paths/cartesian_global_dec18_120_fixations/sc_trial_%d_image_%d.csv'%(trial_id, num), saccade_path[0:counter])
