...
info = h5info(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num));

Wg_x = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wg_x')');
Wg_s = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wg_s')');
Wg_g = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wg_g')');
Wg = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wg')');
Wx = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wx')');
Wh = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wh')');
Wo = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/Wo')');
bo = double(h5read(sprintf('../Weights_withII/weights_epoch%d.h5', epoch_num), '/bo')');
...
gi_frame = zeros(xsize, ysize);
gi_frame(ixs(1, :), ixs(2, :)) = stim;

gi_input_flag = 1;
rndNoiseGI = 0;
scale = 1;

%%% simulating <stimuli_sequence> 
per = zeros(size(As));
cd = zeros(size(As));

rtm1s = rand(12500, nN, 4) * 0.0000;
gtm1 = zeros(1, nG);
for t = 1 : nT
    for k = 1 : 4
        Ots = zeros(12500, nX);
        ...
        frame = As(:, :, t);
        if sum(frame(:)) > 0
            xg = imresize(gi_frame, [10, 10], 'nearest');
        else
            xg = zeros(10, 10);
        end
